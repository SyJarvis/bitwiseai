...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python parse_asm.py <asm_file_path> [--json]")
        sys.exit(1)
    ...
    # 解析指令
    instructions = parse_asm_file_to_instructions(file_path)
    
    if not instructions:
        print("解析失败！")
        sys.exit(1)
    
    print(f"\n解析成功！共 {len(instructions)} 条指令\n")
    ...
    if output_json:
        ...
        for instr in instructions:
            # 指令索引显示为二进制格式
            instr_idx_binary = f"0b{instr['instruction_index']:b}"
            print(f"指令 [{instr_idx_binary}]: {instr['instruction_name']}")
            print(f"  原始十六进制: {instr['original_hex']}")
            print(f"  完整指令: {instr['cmd_hex']}")
            print(f"  操作码: {instr['opcode']['hex']} ({instr['opcode']['binary']})")
            print("  字段:")
            for field in instr['fields']:
                if field['name'] != 'reserve':  # 跳过保留位
                    # 构建输出字符串（字段值使用十六进制）
                    value_str = field['value']
                    if 'register_name' in field:
                        value_str += f" -> {field['register_name']}"
                    print(f"    {field['name']:<20} = {value_str:<30} bits: {field['bits']}")
            print()
